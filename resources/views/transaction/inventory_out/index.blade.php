  ...
  <div class="col-12">
    <div class="card">
      <div class="card-header">
        ...
        @if ($user->can('outcoming_inventories.add'))
        <a href="{{url('/outcoming_inventories/create')}}" class="btn btn-primary d-none d-sm-inline-block">
...
@section('modal')
@include('transaction.inventory_out.modal')
...
@section('script')
  <script type="text/javascript">
    ...
    $(document).on('click', '.edit-ba', function (e) { 
        ...
        getCustomers()
        const id = $(this).data('id')
        const model_type = "App\\Models\\OutcomingInventories"
        const data = {
          model_id:id,
          model: model_type
        };
    ...
    function getCustomers() {
        $.ajax({
            url: BASE_URL+"/api/customers?all=true",
            type: 'GET',
            "headers": {
              'Authorization': TOKEN
            },
            ...
            success: function(data, textStatus, jqXHR){
                let content = '';
                content += '<option value="" selected disabled>-- Choose Customer --</option>';
    ...
    function drawDatatable() {
      $("#main-table").DataTable({
        ...
        // "ordering": false,
        "ajax":{
            "url": BASE_URL+"/api/outcoming_inventory_datatables",
            "headers": {
              'Authorization': TOKEN
            },
            "dataType": "json",
  ...
  $(document).on('click', 'a#delete-data', function( e ) {
    ...
    swal( {
      ...
      $.ajax({
        url: BASE_URL + '/api/outcoming_inventories/' + id,
        "headers": {
          'Authorization': TOKEN
        },
        type: "DELETE"
